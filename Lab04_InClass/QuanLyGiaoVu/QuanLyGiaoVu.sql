...
SELECT MAKHOA, COUNT(MAGV) AS SoGiaoVien
FROM GIAOVIEN
...
INNER JOIN GIANGDAY ON GIANGDAY.MAGV = GIANGDAY.MAGV AND LOP.MALOP = GIANGDAY.MALOP
...
WHERE HOCVIEN.MAHV IN (SELECT TRGLOP FROM LOP)
GROUP BY HOCVIEN.MAHV, HOCVIEN.HO, HOCVIEN.TEN
HAVING SUM(CASE WHEN KETQUATHI.KQUA = 'Không đạt' THEN 1 ELSE 0 END) <= 3;
...
FROM (
    SELECT HOCVIEN.MALOP, HOCVIEN.MAHV, HOCVIEN.HO, HOCVIEN.TEN, COUNT(*) AS SoMonDiemCao,
           RANK() OVER (PARTITION BY HOCVIEN.MALOP ORDER BY COUNT(*) DESC) AS XepHang
    FROM HOCVIEN 
    INNER JOIN KETQUATHI ON HOCVIEN.MAHV = KETQUATHI.MAHV
    WHERE KETQUATHI.DIEM IN (9, 10)
...
WHERE NOT EXISTS (
    SELECT 1
    FROM KETQUATHI 
    WHERE KETQUATHI.MAHV = HOCVIEN.MAHV AND KETQUATHI.LANTHI = 1 AND KETQUATHI.KQUA = 'Không đạt');
