...
SELECT HoTen, Email FROM ChuyenGia WHERE DATEDIFF(YEAR, NgaySinh, 2024) < 35;
...
SELECT KyNang.MaKyNang, KyNang.TenKyNang, KyNang.LoaiKyNang, COUNT(ChuyenGia_KyNang.MaChuyenGia) AS SoLuongChuyenGia
FROM KyNang
INNER JOIN ChuyenGia_KyNang ON ChuyenGia_KyNang.MaKyNang = KyNang.MaKyNang
WHERE ChuyenGia_KyNang.CapDo >= 4
GROUP BY KyNang.MaKyNang, KyNang.TenKyNang, KyNang.LoaiKyNang
...
SELECT MaCongTy, TenCongTy, LinhVuc, AVG(SoNhanVien) AS TrungBinhSoNhanVien
FROM CongTy
WHERE SoNhanVien BETWEEN 50 AND 150
GROUP BY MaCongTy, TenCongTy, LinhVuc
ORDER BY TenCongTy
...
SELECT ChuyenGia_KyNang.MaChuyenGia, HoTen , COUNT(MaKyNang) as SoKyNang 
...
SELECT ChuyenGia_KyNang.MaKyNang, TenKyNang, COUNT(MaChuyenGia) AS SoChuyenGia 
FROM ChuyenGia_KyNang
INNER JOIN KyNang ON KyNang.MaKyNang = ChuyenGia_KyNang.MaKyNang
WHERE CapDo >= 4
GROUP BY ChuyenGia_KyNang.MaKyNang, TenKyNang
...
SELECT HoTen , AVG(CapDo) AS TrungBinhCapDoKyNang
FROM ChuyenGia
