...
CREATE TRIGGER trg_ngaycapnhat_chuyengia
ON ChuyenGia
AFTER UPDATE
AS
BEGIN
...
CREATE TRIGGER trg_ghilog_DuAn
ON DuAn
AFTER INSERT, UPDATE, DELETE
AS
BEGIN
	IF EXISTS (SELECT 1 FROM INSERTED) AND EXISTS (SELECT 1 FROM DELETED)
		PRINT 'Bảng DuAn đã được cập nhật'
	ELSE IF EXISTS (SELECT 1 FROM INSERTED)
		PRINT 'Bản ghi đã được thêm vào bảng DuAn'
	ELSE IF EXISTS (SELECT 1 FROM deleted)
		PRINT 'Bản ghi đã được xóa khỏi bảng DuAn'
END;

-- 103. Tạo một trigger để đảm bảo rằng một chuyên gia không thể tham gia vào quá 5 dự án cùng một lúc.
CREATE TRIGGER trg_thamgiaqua5duan_chuyengiaduan
ON ChuyenGia_DuAn
AFTER INSERT, UPDATE
AS
BEGIN
    IF EXISTS (
		SELECT *
		FROM inserted I
		WHERE (SELECT COUNT(*) FROM ChuyenGia_DuAn WHERE MaChuyenGia = I.MaChuyenGia) > 5)
    BEGIN
        ROLLBACK TRANSACTION
        PRINT 'Chuyên gia không thể tham gia vào quá 5 dự án cùng một lúc.'
    END
END;
...
BEGIN
    ...
    BEGIN
		...
		SET SoNhanVien += (
			SELECT COUNT(*) 
			FROM inserted I
			INNER JOIN ChuyenGia_DuAN CG_DA ON CG_DA.MaChuyenGia = I.MaChuyenGia
			INNER JOIN DuAn DA ON DA.MaDuAN = CG_DA.MaDuAn
			WHERE EXISTS (
				SELECT 1 
				FROM CongTy
				WHERE CongTy.MaCongTy = DA.MaCongTy)) 
    END
...
CREATE TRIGGER trg_capdokynang
ON ChuyenGia_DuAn
AFTER INSERT
AS
BEGIN
...
CREATE TRIGGER trg_ngayketthuc_ngaybatdau
ON DuAn
AFTER INSERT, UPDATE
AS
BEGIN
...
CREATE TRIGGER trg_khongquamuoiduan
ON DuAn
AFTER INSERT
AS
BEGIN
    ...
    BEGIN
        ROLLBACK TRANSACTION
        PRINT 'Công ty không thể có quá 10 dự án đang thực hiện cùng một lúc.'
    END
END;
...
CREATE TRIGGER trg_capnhatluong_chuyengia
ON ChuyenGia
AFTER INSERT, UPDATE
AS
BEGIN
	...
	FROM ChuyenGia CG
	INNER JOIN INSERTED I ON I.MaChuyenGia =  CG.MaChuyenGia
	INNER JOIN ChuyenGia_KyNang CGKN ON CGKN.MaChuyenGia = I.MaChuyenGia
END;
...
BEGIN
    IF EXISTS (
        SELECT 1 
        FROM deleted D
        INNER JOIN ChuyenGia_DuAn CG_DA ON D.MaChuyenGia = CG_DA.MaChuyenGia
    )
    BEGIN
